...
function MyPurchaseHistory() {
    ...
    const getOrders = async () => {
      ...
      const orderList = data.docs
        ...
        .filter(
          ...
          || 
          (cart.Email === sessionStorage.getItem("email")
          &&
  ...
  const sumOfPrice = useMemo(() => {
    ...
    const delivered = orders.filter(deliveredItem => deliveredItem.Status === "Delivered")
    delivered.forEach((orderItem) => (totalAmount += orderItem.totalAmount));
    return totalAmount;
  }, [orders]);
    ...
    <div>
      ...
      <div className="main-content-container">
        ...
        <div className="content-container">
                    ...
                    >
                      ...
                      <div style={{display: 'flex', flexDirection: 'row', textAlign: 'center'}}>
                        <div className="tooltip">
                          <DateRangeIcon />
                          <span class="tooltiptext"> <sub>Date: {order.orders[0].Date} </sub></span>
                        </div>
                        <h4 style={{padding: 0, margin: 0}}>| Status: {order.orders[0].Status} </h4>
                      </div>
                    </div>
                  </div>
                          ...
                          <Paper elevation={2} className="purchase-border">
                            <div className="purchase">
                              <div className="img-container">
                                <img src={actualOrder.ImageURL} alt="Product-image" />
                              </div>
          ...
          <div className="total">
            <p>
              Total spend: <span> ₱ {sumOfPrice.toLocaleString()}</span>
            </p>
          </div>
        </div>
      </div>
